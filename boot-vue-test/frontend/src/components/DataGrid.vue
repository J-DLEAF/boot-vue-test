    ...
    <v-app id="list-sample">
        ...
        <v-list two-line v-for="(listItem, index) in calData" :key="index">
          <v-list-tile>
            <v-list-tile-content>
              ...
              <v-list-tile-sub-title>{{ listItem.desc }}</v-list-tile-sub-title>
            </v-list-tile-content>
        ...
        <base-modal>
          ...
          <v-container slot="content" grid-list-md>
            <v-layout wrap>
              ...
              <v-flex xs12>
                <v-text-field label="Desc" v-model="newData.desc" @keydown.enter="submit" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
          <v-btn slot="negative-button" color="blue darken-1" flat @click="closeDialog">Close</v-btn>
          <v-btn slot="positive-button" color="blue darken-1" flat @click="submit">Save</v-btn>
        </base-modal>
      </v-dialog>
      ...
      <v-dialog v-model="editDialog" persistent max-width="500px" @keydown.esc="closeDialog">
        <base-modal>
          ...
          <v-btn slot="negative-button" color="blue darken-1" flat @click="closeDialog">Close</v-btn>
          <v-btn slot="positive-button" color="blue darken-1" flat @click="editData">Save</v-btn>
        </base-modal>
      </v-dialog>
      ...
      >
        <base-modal>
          ...
          <v-btn slot="negative-button" color="blue darken-1" flat @click="closeDialog">Cancel</v-btn>
          <v-btn slot="positive-button" color="blue darken-1" flat @click="removeData">Confirm</v-btn>
        </base-modal>
      </v-dialog>
    </v-app>
  </div>
